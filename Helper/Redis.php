...
class Redis extends \Litmus7\Merc\Helper\Backend implements \Litmus7\Merc\Helper\ExtendedInterface
{
    ...
    public function __construct(\Magento\Framework\App\DeploymentConfig $deploymentConfig)
    {
        ...
        if ( empty($options['server']) ) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Redis \'server\' not specified.'));
        }

        if ( empty($options['port']) && substr($options['server'],0,1) != '/' ) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Redis \'port\' not specified.'));
        }
        ...
        $this->_redis   = new client($options['server'], $port , $timeout, $persistent,$options['database']);
        ...
        if(! $this->_redis->select( $options['database']))
    ...
    public function clean($mode = Zend_Cache::CLEANING_MODE_ALL, $tags = array())
    {
        ...
        try {
            ...
            switch ($mode)
            {
                ...
                default:
                    throw new \Magento\Framework\Exception\LocalizedException(__('Invalid mode for clean() method: '.$mode));
            }
        } catch (\CredisException $e) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Error cleaning cache by mode '.$mode.': '.$e->getMessage(), $e));
        }
        return TRUE;
    }
    ...
    public function setDirectives($directives)
    {
        ...
        if ($lifetime > self::MAX_LIFETIME) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Redis backend has a limit of 30 days (2592000 seconds) for the lifetime'));
        }
    }
    ...
    {
        if( ! $this->_notMatchingTags) {
            throw new \Magento\Framework\Exception\LocalizedException(__("notMatchingTags is currently disabled."));
        }
        if ($tags) {
